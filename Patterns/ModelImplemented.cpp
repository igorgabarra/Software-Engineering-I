...
System* ModelImplemented::createSystem(string name, double value){
	System* system = new SystemImplemented(name, value);
	add(system);
	return system;
}

ModelImplemented* ModelImplemented::instance = NULL;

bool ModelImplemented::deleteSystem(System* del){
	System* auxiliar = getSystem(del->getName());
	
	if(auxiliar != NULL){
		removeSystem(del->getName());
		delete (SystemImplemented*) auxiliar;
		return true;
	}
...
Model* ModelImplemented::createModel(string name){
	if (instance == NULL)
		instance = new ModelImplemented(name); 
        
	return instance;
}


bool ModelImplemented::deleteModel(Model* model){
	if(model != NULL){
		delete (ModelImplemented*) model;
		instance = NULL;
		return true;
	}
	return false;
